...
ovpn_client_rename_command() {
  local usage='usage: ovpn client rename <name>|--id|-i <ID>|--name|-n <NAME> <new-name>'
...
ovpn_client_revoke_command() {
  local usage='usage: ovpn client revoke <name>|--id|-i <ID>|--name|-n <NAME> [--release-ip|-r]'
...
ovpn_client_release_ip_command() {
  local usage='usage: ovpn client ip release <name>|--id|-i <ID>|--name|-n <NAME>'
  local selector_mode reference consumed
...
ovpn_client_reissue_command() {
  local usage='usage: ovpn client reissue <name>|--id|-i <ID>|--name|-n <NAME> [--dynamic|-d|--ip|-I <IPv4>]'
  local selector_mode reference consumed
...
ovpn_client_delete_command() {
  local usage='usage: ovpn client delete <name>|--id|-i <ID>|--name|-n <NAME>'
  local selector_mode reference consumed
...
ovpn_client_command() {
  ...
  case "$subcommand" in
    ...
    export)
      if ovpn_help_requested "$@"; then
        ovpn_command_usage "ovpn client export <name>|--id|-i <ID>|--name|-n <NAME>" "Render an active client profile to stdout."
      else
    ...
    ip)
      ...
      case "$ip_subcommand" in
        set)
          if ovpn_help_requested "$@"; then
            ovpn_command_usage "ovpn client ip set <name...>|--id|-i <ID>|--name|-n <NAME>|--all|-a [--dynamic|-d|--ip|-I <IPv4>]" "Assign client IP addresses."
          else
        ...
        release)
          if ovpn_help_requested "$@"; then
            ovpn_command_usage "ovpn client ip release <name>|--id|-i <ID>|--name|-n <NAME>" "Release the retained static IP of a revoked client."
          else
            ovpn_client_release_ip_command "$@"
          fi
          ;;
    ...
    revoke)
      if ovpn_help_requested "$@"; then
        ovpn_command_usage "ovpn client revoke <name>|--id|-i <ID>|--name|-n <NAME> [--release-ip|-r]" "Revoke a client certificate and optionally release its static IP."
      else
        ovpn_client_revoke_command "$@"
      fi
      ;;
    reissue)
      if ovpn_help_requested "$@"; then
        ovpn_command_usage "ovpn client reissue <name>|--id|-i <ID>|--name|-n <NAME> [--dynamic|-d|--ip|-I <IPv4>]" "Issue a new certificate for an existing client, optionally changing IP assignment."
      else
        ovpn_client_reissue_command "$@"
      fi
      ;;
    delete)
      if ovpn_help_requested "$@"; then
        ovpn_command_usage "ovpn client delete <name>|--id|-i <ID>|--name|-n <NAME>" "Remove a client and its local credentials."
      else
        ovpn_client_delete_command "$@"
      fi
      ;;
    ...
    rename)
      if ovpn_help_requested "$@"; then
        ovpn_command_usage "ovpn client rename <name>|--id|-i <ID>|--name|-n <NAME> <new-name>" "Change a client's display name without replacing its UUID or certificate."
      else
        ovpn_client_rename_command "$@"
      fi
      ;;
