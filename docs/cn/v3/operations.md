...
管理代码、镜像/platform、OpenVPN 和持久化数据 schema 相互独立。更新前先查看
active 值：
...
可用 `--version X.Y.Z` 选择指定的更高稳定版本。下载、签名验证、解包和隔离自检都在
active 指针切换前完成；命令不会修改实例数据、重载 OpenVPN 或断开客户端。若最新
Release 不兼容，自动选择会尝试最高的兼容更新，并报告跳过目标。platform API 或
OpenVPN 不匹配时必须升级镜像；schema 不匹配时必须执行下述停服迁移。

只能回滚到仍兼容且被保留的 previous bundle：

```bash
docker compose exec openvpn ovpn upgrade --rollback --yes
```

rollback 只切换管理代码，不能撤销配置变更或数据迁移。
...
2. 只读查看迁移计划：
   ...
   若要在同一事务中部署更新的已签名管理版本，在 plan 和 apply 中都增加
   `--to-version X.Y.Z`。目标需要更新 platform API 或 OpenVPN runtime 时，应先
   更新镜像。
...
3. 通过 maintenance 执行：
   ...
   apply 要求服务持续停止并获取独占锁。它创建数据目录快照（排除内部 bundle
   存储 `repair/.scripts`），在独立 staging 中迁移并验证目标，最后共同提交代码和
   数据；失败会恢复二者指针，中断事务由下一次 apply 恢复。报告和快照保留在
...
`--raw`。`runtime events` 提供连接、生命周期、rename、IP、网络、在线更新和数据
