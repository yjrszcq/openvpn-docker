...
每个选中客户端必须恰好出现一次，值只能是 `auto`、`dynamic` 或静态 IPv4。位置名称、`--name`、`--id` 与 `--all` 不能混用。完整集合统一验证并原子提交，因此支持地址交换。编辑器依次取 `--editor/-e`、`OVPN_EDITOR`、`EDITOR`、已安装的 `nano`。镜像可直接使用 `nano`、`vim` 和 `vi`，也可以选择其他已经安装或挂载到容器内的可执行文件。参数值只能是单个可执行文件名或路径，不能包含额外参数。提交后会断开选中的在线 session。
