...
1. 创建权限受限的数据与配置目录。
2. 将仓库根目录的 `config.example.yaml` 复制为 `config/config.yaml`，再确认 endpoint、IPv4 网段、路由和 NAT。
...
这些脚本补全的是名为 `ovpn` 的直接命令。可在服务容器的交互 shell 中运行，或在宿主机定义同名 wrapper，内部调用 `docker exec openvpn ovpn`。从宿主机生成时，把下面的 `ovpn completion` 换成 `docker exec openvpn ovpn completion`。
...
验证完成前保留 `data-schema4`。恢复后的 schema 3 数据树与 `sh-ver` 镜像必须作为匹配单元使用。
