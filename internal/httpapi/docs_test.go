...
func TestDocumentationSupportsCompleteChineseAndEnglishRendering(t *testing.T) {
	...
	page := string(index)
	for _, expected := range []string{`data-language="zh"`, `data-language="en"`, `/docs/i18n.js`, `id="configuration-table"`, `id="api-key-table"`, `ovpn api key create frontend-production`, `ovpn api key list`, `ovpn api key delete frontend-production --yes`} {
	...
	if strings.Count(page, "docker exec openvpn") != 1 || !strings.Contains(page, `docker exec openvpn ovpn &lt;command&gt;`) {
		t.Fatal("documentation must state the host command convention once without repeating it in the table")
	}
	...
	for _, expected := range []string{`title: "API 接口文档"`, `title: "API Reference"`, `tableHeaders: ["字段"`, `tableHeaders: ["Field"`, `configurationHeaders: ["变量", "默认值", "可填写值", "说明"]`, `configurationHeaders: ["Variable", "Default", "Accepted values", "Description"]`, `apiKeyHeaders: ["命令", "说明"]`, `apiKeyHeaders: ["Command", "Description"]`, `命令格式约定为`, `When running commands from the host`, `完整 secret 只会显示一次`, `prints its complete secret once`, `127.0.0.1:<空闲端口>`, `0.0.0.0:<unused-port>`, `vpn-admin.example.com,192.0.2.10:3000`, `单独填写 *`, `Use * alone`} {
