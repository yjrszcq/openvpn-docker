...
func TestVersionJSON(t *testing.T) {
	...
	if info.Version != buildinfo.Version || info.DataSchema != buildinfo.DataSchema || info.GoVersion == "" || info.Dependencies.SQLite == "" || info.Dependencies.YAML == "" {
...
func TestTopLevelVersionAliases(t *testing.T) {
	...
	if code != 0 || strings.TrimSpace(stdout) != buildinfo.Version || stderr != "" {
	...
	for _, alias := range []string{"-V", "--version"} {
		...
		if code != 0 || aliasError != "" || !strings.Contains(aliasOutput, "ovpn "+buildinfo.Version) || !strings.Contains(aliasOutput, "data schema: "+strconv.Itoa(buildinfo.DataSchema)) {
...
func TestEntrypointDispatchesOVPNAndDefaultCommands(t *testing.T) {
	...
	if code != 0 || strings.TrimSpace(stdout.String()) != buildinfo.Version || stderr.Len() != 0 {
