	...
	children: []command{
		...
		group("config", "validate, inspect, plan, or apply declarative configuration",
			...
			leaf("plan", "plan desired-to-applied configuration changes", "ovpn config plan [--json|-j]",
				...
				"ovpn config plan"),
			leaf("apply", "apply configuration and restart the managed runtime", "ovpn config apply [--yes|-y] [--json|-j]",
				[]string{"The live supervisor pauses OpenVPN and the broker, grants the exclusive runtime lock, then restarts them. Offline maintenance remains supported."},
