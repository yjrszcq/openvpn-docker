...
var rootCommand = command{
	...
	children: []command{
		group("api", "manage REST API credentials",
			group("key", "create, list, or delete API keys",
				leaf("create", "create an API key", "ovpn api key create NAME [--output|-o FILE|-] [--json|-j]",
					[]string{"The secret is shown once. File output uses mode 0600 and is never overwritten."},
					"ovpn api key create frontend", "ovpn api key create frontend -o api.key"),
				leaf("list", "list API keys", "ovpn api key list [--json|-j]",
					[]string{"Secrets and digests are never returned."},
					"ovpn api key list"),
				leaf("delete", "delete an API key", "ovpn api key delete (NAME|--name|-n NAME|--id|-i ID) [--yes|-y] [--json|-j]",
					[]string{"Deletion immediately invalidates the key and requires confirmation."},
					"ovpn api key delete frontend", "ovpn api key delete --id 550e8400 --yes"))),
		...
		group("config", "validate, inspect, plan, or apply declarative configuration",
			...
			leaf("plan", "plan desired-to-applied configuration changes", "ovpn config plan [--json|-j]",
				...
				"ovpn config plan"),
			leaf("apply", "apply configuration and restart the managed runtime", "ovpn config apply [--yes|-y] [--force|-f] [--json|-j]",
				[]string{"Requires a healthy state preflight unless --force or -f is supplied. Force bypasses only the preflight result.", "The live supervisor pauses OpenVPN and the broker, grants the exclusive runtime lock, then restarts them. Offline maintenance remains supported."},
				"ovpn config apply", "ovpn config apply -y -j", "ovpn config apply -y -f")),
		group("client", "manage client identities, profiles, and addresses",
			...
			group("address", "manage client IPv4 assignment intent",
				...
				leaf("edit", "edit multiple client IPv4 assignments atomically", "ovpn client address edit (--all|-a|NAME...|--name|-n NAME...|--id|-i ID...) [--editor|-e EDITOR] [--yes|-y] [--json|-j]",
					[]string{"Opens a private CSV using --editor/-e, OVPN_EDITOR, EDITOR, then nano. The editor must be one executable name or path. Requires confirmation.", "Selected active sessions are disconnected after the batch commits."},
					"ovpn client address edit laptop phone", "ovpn client address edit -a -e vim -y"),
