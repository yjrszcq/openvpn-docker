...
| `OVPN_NETWORK` | `10.8.0.0/24` | IPv4 隧道网段，必须选择无重叠且规范的 CIDR。 |
| `OVPN_TOPOLOGY` | `subnet` | 必须使用的 IPv4 拓扑；不接受其他拓扑。 |
| `OVPN_DYNAMIC_POOL_SIZE` | 可用客户端地址数的一半 | 从可用地址尾部划出的动态池；0 和全部可用地址都是合法边界。 |
...
一致性备份应包含 config/、data/、meta/、pki/、secrets/、ccd/ 与 clients/，
以及独立挂载的 ./openvpn-runtime/pool-persist.txt 动态租约文件。审计日志和已应用清单快照
位于 meta/；不得把私钥复制到工单、日志或临时恢复记录。推荐在停止服务后备份；恢复两个卷后，
先运行 doctor，审阅 repair --plan，再启动服务。
