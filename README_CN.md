...
maintenance-only 迁移和发布要求在命令文档换版后仍持续有效。

管理代码、镜像、OpenVPN 和数据 schema 的独立版本及在线更新边界遵循
[管理代码在线更新政策](docs/cn/management-update-policy.md)。在线管理更新不会替换
OpenVPN 内核，也不会迁移持久化数据。
