...
"$OVPN" config init
"$OVPN" config print >"$TMP_DIR/default.out"
...
OVPN_DATA_DIR="$TMP_DIR/static" "$OVPN" config print >"$TMP_DIR/static.out"
grep -Fqx 'OVPN_DYNAMIC_POOL_SIZE=0' "$TMP_DIR/static.out"

OVPN_DATA_DIR="$TMP_DIR/dynamic" OVPN_DYNAMIC_POOL_SIZE=253 "$OVPN" config init
OVPN_DATA_DIR="$TMP_DIR/dynamic" "$OVPN" config print >"$TMP_DIR/dynamic.out"
grep -Fqx 'OVPN_DYNAMIC_POOL_SIZE=253' "$TMP_DIR/dynamic.out"

if OVPN_DATA_DIR="$TMP_DIR/invalid-pool" OVPN_DYNAMIC_POOL_SIZE=254 "$OVPN" config init >"$TMP_DIR/invalid-pool.out" 2>"$TMP_DIR/invalid-pool.err"; then
...
if OVPN_DATA_DIR="$TMP_DIR/invalid-network" OVPN_NETWORK=10.88.0.1/24 "$OVPN" config init >"$TMP_DIR/invalid-network.out" 2>"$TMP_DIR/invalid-network.err"; then
...
if OVPN_DATA_DIR="$TMP_DIR/small-network" OVPN_NETWORK=10.88.0.0/31 "$OVPN" config init >"$TMP_DIR/small-network.out" 2>"$TMP_DIR/small-network.err"; then
