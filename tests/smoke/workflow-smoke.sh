...
grep -Fq 'tests/smoke/ipam-layout-smoke.sh' "$WORKFLOWS/test.yml"
grep -Fq 'tests/smoke/registry-migration-smoke.sh' "$WORKFLOWS/test.yml"
grep -Fq 'tests/smoke/client-ip-set-smoke.sh' "$WORKFLOWS/test.yml"
grep -Fq 'tests/smoke/ipam-repair-smoke.sh' "$WORKFLOWS/test.yml"
grep -Fq 'tests/smoke/network-migration-smoke.sh' "$WORKFLOWS/test.yml"
grep -Fq 'OVPN_NETWORK_MIGRATION_REQUIRED=1' "$WORKFLOWS/test.yml"
grep -Fq 'linux/amd64,linux/arm64' "$WORKFLOWS/test.yml"
grep -Fq 'tests/container/upgrade-state-smoke.sh' "$WORKFLOWS/test.yml"
grep -Fq 'tests/smoke/license-smoke.sh' "$WORKFLOWS/test.yml"
