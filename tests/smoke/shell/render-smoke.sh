...
grep -q "^ifconfig-pool-persist $OVPN_DATA_DIR/data/pool-persist.txt 0$" "$TMP_DIR/server.conf"
grep -q '^script-security 2$' "$TMP_DIR/server.conf"
grep -q "^client-connect /usr/local/lib/openvpn-container/pool-persist-hook.sh $OVPN_DATA_DIR/data/pool-persist.txt$" "$TMP_DIR/server.conf"
grep -q "^client-disconnect /usr/local/lib/openvpn-container/pool-persist-hook.sh $OVPN_DATA_DIR/data/pool-persist.txt$" "$TMP_DIR/server.conf"
