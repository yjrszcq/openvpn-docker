...
grep -q "^client-connect /usr/local/lib/openvpn-container/pool-persist-hook.sh $OVPN_DATA_DIR/data/leases$" "$TMP_DIR/server.conf"
grep -q "^client-disconnect /usr/local/lib/openvpn-container/pool-persist-hook.sh $OVPN_DATA_DIR/data/leases$" "$TMP_DIR/server.conf"
