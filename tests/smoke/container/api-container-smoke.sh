...
grep -Fq 'docker exec openvpn ovpn api key create frontend-production' "$WORK_DIR/docs.html"
grep -Fq 'docker exec openvpn ovpn api key list' "$WORK_DIR/docs.html"
grep -Fq 'docker exec openvpn ovpn api key delete frontend-production --yes' "$WORK_DIR/docs.html"
