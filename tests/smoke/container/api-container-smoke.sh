...
curl -fsS "$api_url/docs/i18n.js" >"$WORK_DIR/docs-i18n.js"
...
grep -Fq 'id="api-key-table"' "$WORK_DIR/docs.html"
grep -Fq 'ovpn api key create frontend-production' "$WORK_DIR/docs.html"
grep -Fq 'ovpn api key list' "$WORK_DIR/docs.html"
grep -Fq 'ovpn api key delete frontend-production --yes' "$WORK_DIR/docs.html"
grep -Fq 'docker exec openvpn ovpn &lt;command&gt;' "$WORK_DIR/docs.html"
if [ "$(grep -Fo 'docker exec openvpn' "$WORK_DIR/docs.html" | wc -l)" -ne 1 ]; then
  printf 'REST API smoke failed: host command convention must appear once without table repetition\n' >&2
  exit 1
fi
...
grep -Fq 'apiKeyHeaders: ["命令", "说明"]' "$WORK_DIR/docs-i18n.js"
grep -Fq '命令格式约定为' "$WORK_DIR/docs-i18n.js"
grep -Fq '完整 secret 只会显示一次' "$WORK_DIR/docs-i18n.js"
