...
run_control add-client recovery-client >/tmp/ovpn-recovery-add.out 2>/tmp/ovpn-recovery-add.err
...
if [ "$recovery_state" != DEGRADED_RECOVERABLE ]; then
  run_control doctor --json >&2 || true
...
run_control repair >/tmp/ovpn-recovery-repair.out 2>/tmp/ovpn-recovery-repair.err
