...
grep -Fq "\`IP STATE\`, and \`CONNECTION\` columns" "$EN_README"
grep -Fq 'ovpn client list --ip' "$EN_README"
grep -Fq 'A dynamic IP is informational, never a reservation' "$EN_README"
grep -Fq "still occupies its reservation is \`retained\`" "$EN_README"
...
grep -Fq "\`IP STATE\` 和 \`CONNECTION\` 列" "$CN_README"
grep -Fq 'ovpn client list --ip' "$CN_README"
grep -Fq '动态 IP 仅为状态信息，绝不是地址保留' "$CN_README"
grep -Fq "静态客户端为 \`retained\`" "$CN_README"
